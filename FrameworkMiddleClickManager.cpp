/**
 * @file      FrameworkMiddleClickManager.cpp
 * @brief     FrameworkMiddleClickManager 模块的逻辑实现。
 * @author    IYATT-yx
...
FrameworkMiddleClickManager::FrameworkMiddleClickManager() : mhUnifiedMiddleClickHook(nullptr)
{
}

FrameworkMiddleClickManager::~FrameworkMiddleClickManager()
{
    this->stopUnifiedMiddleClickProc();
}

FrameworkMiddleClickManager& FrameworkMiddleClickManager::getInstance()
{
    static FrameworkMiddleClickManager instance;
    return instance;
}

void FrameworkMiddleClickManager::startUnifiedMiddleClickProc(ConfigItems::MiddleClickManagerSettings& settings)
{
...
void FrameworkMiddleClickManager::stopUnifiedMiddleClickProc()
{
    if (this->mhUnifiedMiddleClickHook != nullptr)
    {
...
LRESULT CALLBACK FrameworkMiddleClickManager::unifiedMiddleClickProc(int nCode, WPARAM wParam, LPARAM lParam)
{
    if (nCode == HC_ACTION)
    {
        auto& instance = FrameworkMiddleClickManager::getInstance();
        ...
        if (wParam == WM_MBUTTONUP)
        {
            ...
            if (instance.mbEnabledCmdEnter)
            {
                ...
                UtilAcadVar::getVar(L"CMDACTIVE", cmdActive);
                if (cmdActive > 0 && dwDuration < instance.mdCmdMiddleClickDownUpInterval)
                {
                    static FrameworkCommands::CommandList space = { L"" };
                    FrameworkCommands::executeCommand(space, false);
