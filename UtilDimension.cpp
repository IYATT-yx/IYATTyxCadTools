...
namespace UtilDimension
{
	void dimensionSolidify(const AcDbObjectId& objId)
	{
		...
		UtilDimension::DimensionData dimData{}; // 存储标注数据
	...
	void setAndUnsetRefDim(const AcDbObjectId& objId, bool isSet)
	{
		...
		if (isSet)
		{
			UtilDimension::addSurroundingCharsForDimension(objId, left, right);
		}
		else
		{
            UtilDimension::removeSurroundingCharsForDimension(objId, left, right);
		}
	}
	...
	void readDim(const AcDbObjectId& id, DimensionData& data)
	{
		...
		if (!UtilDimension::isEnhancedToleranceApplied(data.dimensionText))
		{
			pDim->setDimtp(0);
			pDim->setDimtm(0);
		}
