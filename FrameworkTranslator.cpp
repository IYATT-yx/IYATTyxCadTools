...
module FrameworkTranslator;
import FrameworkConfigManager;
import FrameworkCommands;

FrameworkTranslator& FrameworkTranslator::getInstance()
{
	static FrameworkTranslator instance;
	return instance;
}

bool FrameworkTranslator::initialize(const std::filesystem::path& localesPath, const std::wstring& languageCode)
{
...
const wchar_t* FrameworkTranslator::translate(const wchar_t* text, const std::wstring& domain) const
{
	if (!this->mStatus)
	{
		{
			throw std::runtime_error("FrameworkTranslator is not initialized.");
		}
	}
...
std::unordered_map<std::wstring, std::wstring>FrameworkTranslator::loadMoFile(const std::filesystem::path& path, const std::wstring& domain) const
{
	...
	for (std::uint32_t i = 0; i < stringCount; ++i)
	{
		...
		std::wstring wOrig = UtilIoEncodingConverter::FromUtf8ToWstring(origStr);
		std::wstring wTrans = UtilIoEncodingConverter::FromUtf8ToWstring(transStr);
