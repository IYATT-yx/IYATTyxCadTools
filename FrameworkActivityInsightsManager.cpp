/**
 * @file      FrameworkActivityInsightsManager.cpp
...
module FrameworkActivityInsightsManager;
import std;
...
std::vector<CadHistory::ActivityItem> FrameworkActivityInsightsManager::fetchHistory(CadHistory::OpFilter eFilter, bool bUnique, bool bFilterExisting)
{
...
std::wstring FrameworkActivityInsightsManager::getCommonFolderPath()
{
...
bool FrameworkActivityInsightsManager::isOpMatched(const std::wstring& wstrOp, CadHistory::OpFilter eFilter)
{
...
std::vector<CadHistory::ActivityItem> FrameworkActivityInsightsManager::parseJsonFileShared(const std::wstring& wstrPath, CadHistory::OpFilter eFilter)
{
    ...
    if (dwSize > 0)
    {
        ...
        if (::ReadFile(hFile, &strRawContent[0], dwSize, &dwBytesRead, nullptr))
        {
            try
            {
                UtilIoJsonWrapper::json jsonDoc = UtilIoJsonWrapper::json::parse(strRawContent);
                if (jsonDoc.is_array())
                {
                    for (const auto& jsonNode : jsonDoc)
                    {
                        // 隐式触发由 UtilIoJsonWrapper 注入到 std 命名空间下的 from_json 管道实现
