/**
 * @file      FrameworkConfigManager.ixx
...
export module FrameworkConfigManager;
...
public:
	/**
	* @brief 获取 FrameworkConfigManager 全局唯一实例。
	* @return FrameworkConfigManager 引用。
	*/
	static FrameworkConfigManager& getInstance();

	// 禁止拷贝和赋值操作
	FrameworkConfigManager(const FrameworkConfigManager&) = delete;
    FrameworkConfigManager& operator=(const FrameworkConfigManager&) = delete;

	ConfigItems::GlobalConfig& getConfig();

private:
	FrameworkConfigManager() = default;
...
public:
	...
	* UtilIoJsonWrapper 的自愈机制创建一个空的 JSON 对象。加载成功后，
