...
void UiUniversalPicker::batchSelect(UiUniversalPicker::AcRxClassVectorPtr arcv, UiUniversalPicker::EntityProcessorBreak processor, SortMode defaultSortMode, bool isLocked, double sortTol)
{
    ads_name ss;
    ...
    {
        UiUniversalPicker::freeFilter(filterRb);
        return;
    }
    ...
    if (!isLocked)
    {
        ...
        wchar_t kword[20];
        AcString asKword;
        const wchar_t* prefix = _(L"\n排序模式 [右下(RD)/右上(RU)/左下(LD)/左上(LU)/下右(DR)/下左(DL)/上右(UR)/上左(UL)/无(None)] <");
        const wchar_t* suffix = L">: ";
        asKword.format(L"%s%s%s", prefix, UiUniversalPicker::SortModeToString(finalMode), suffix);
    ...
    try
    {
        ...
        if (finalMode != SortMode::None)
        {
            std::sort(entities.begin(), entities.end(), [finalMode, sortTol](const EntityInfo& a, const EntityInfo& b)
                {
                    return UiUniversalPicker::compareEntities(a, b, finalMode, sortTol);
                });
...
AcDbObjectId UiUniversalPicker::getSelectedSingleEntityId(UiUniversalPicker::AcRxClassVectorPtr arcv)
{
...
void UiUniversalPicker::immediateSelect(UiUniversalPicker::AcRxClassVectorPtr arcv, UiUniversalPicker::EntityProcessorBreak processor)
{
...
void UiUniversalPicker::run(UiUniversalPicker::AcRxClassVectorPtr arcv, UiUniversalPicker::EntityProcessorBreak processor, const wchar_t* prompt, UiUniversalPicker::SelectMode defaultSelectMode, bool lockSelectMode, SortMode defaultSortMode, bool lockSortMode, double sortTol)
{
    if (prompt != nullptr)
    {
        acutPrintf(L"\n%s\n", prompt);
    ...
    // 模式选择提示
    const wchar_t* modePrompt = (defaultSelectMode == UiUniversalPicker::SelectMode::Batch) ?
        _(L"\n选择模式 [批量(B)/立即(I)]<B>：") :
        _(L"\n选择模式 [批量(B)/立即(I)]<I>：");

    UiUniversalPicker::SelectMode inputMode = defaultSelectMode;
    wchar_t keyword[2] = { 0 };
    ...
    if (!lockSelectMode)
    {
        do
        {
            ...
            if (stat == RTNORM)
            {
                ...
                {
                    inputMode = UiUniversalPicker::SelectMode::Batch;
                    validInput = true;
                }
                else if (_wcsicmp(keyword, L"I") == 0)
                {
                    inputMode = UiUniversalPicker::SelectMode::Immediate;
                    validInput = true;
                }
            }
            else if (stat == RTNONE)
    ...
    if (inputMode == UiUniversalPicker::SelectMode::Batch)
    {
        UiUniversalPicker::batchSelect(arcv, processor, defaultSortMode, lockSortMode, sortTol);
    }
    else
    {
        UiUniversalPicker::immediateSelect(arcv, processor);
    }
}

void UiUniversalPicker::freeFilter(resbuf* filterRb)
{
    if (filterRb != nullptr)
    {
        acutRelRb(filterRb);
...
bool UiUniversalPicker::compareEntities(const UiUniversalPicker::EntityInfo& a, const UiUniversalPicker::EntityInfo& b, UiUniversalPicker::SortMode mode, double sortTol)
{
    switch (mode)
    {
        case UiUniversalPicker::SortMode::RD:
        {
        ...
        case UiUniversalPicker::SortMode::RU:
        {
            if (fabs(a.pos.y - b.pos.y) > sortTol)
            {
                return a.pos.y < b.pos.y;
        ...
        case UiUniversalPicker::SortMode::LD:
        {
            if (fabs(a.pos.y - b.pos.y) > sortTol)
            {
                return a.pos.y > b.pos.y;
        ...
        case UiUniversalPicker::SortMode::LU:
        {
            if (fabs(a.pos.y - b.pos.y) > sortTol)
            {
                return a.pos.y < b.pos.y;
        ...
        case UiUniversalPicker::SortMode::DR:
        {
        ...
        case UiUniversalPicker::SortMode::DL:
        {
            if (fabs(a.pos.x - b.pos.x) > sortTol)
            {
                return a.pos.x > b.pos.x;
        ...
        case UiUniversalPicker::SortMode::UR:
        {
            if (fabs(a.pos.x - b.pos.x) > sortTol)
            {
                return a.pos.x < b.pos.x;
        ...
        case UiUniversalPicker::SortMode::UL:
        {
            if (fabs(a.pos.x - b.pos.x) > sortTol)
            {
                return a.pos.x > b.pos.x;
...
const wchar_t* UiUniversalPicker::SortModeToString(UiUniversalPicker::SortMode mode)
{
    switch (mode)
    {
        case SortMode::RD:
...
void UiUniversalPicker::setSelection(const AcDbObjectIdArray& idArray)
{
    if (idArray.isEmpty())
    {
    ...
    acedSSFree(ss);
    FrameworkCommands::CommandList cmdList =
    {
        L"SELECT"
    };
    FrameworkCommands::executeCommand(cmdList);
}