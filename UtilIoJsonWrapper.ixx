...
export module UtilIoJsonWrapper;
import std;
import UtilIoEncodingConverter;

export namespace UtilIoJsonWrapper
{
...
export namespace std
{
    ...
    * @details 自定义 nlohmann::json 的扩展实现。通过调用 UtilIoEncodingConverter 将 UTF-16 编码的
    ...
    */
    inline void to_json(nlohmann::json& j, const std::wstring& wstr)
    {
        ...
        j = UtilIoEncodingConverter::FromWstringToUtf8(wstr);
    }

    /**
    * @brief 从 JSON 字符串反序列化为宽字符串（std::wstring）。
    * @details 自定义 nlohmann::json 的扩展实现。当捕获到物理文件中的 UTF-8 窄字符串时，
    * 通过 UtilIoEncodingConverter 自动还原为内存专用的 UTF-16 宽字符串。
    ...
    */
    inline void from_json(const nlohmann::json& j, std::wstring& wstr)
    {
        ...
        wstr = UtilIoEncodingConverter::FromUtf8ToWstring(str);
    }
}