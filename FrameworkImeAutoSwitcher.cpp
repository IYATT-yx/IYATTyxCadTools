/**
 * @file      FrameworkImeAutoSwitcher.cpp
 * @brief     FrameworkImeAutoSwitcher 模块的具体逻辑实现。
...
namespace FrameworkImeAutoSwitcher
{
    ...
    {
        int cmdActive = 0;
        if (!UtilAcadVar::getVar(L"CMDACTIVE", cmdActive))
        {
            return false;
        }
        return (cmdActive > 0);
            ...
            if (pKey->vkCode >= 'A' && pKey->vkCode <= 'Z')
            {
                ...
                if (currentTime - g_lastSwitchTime > g_intervalMs)
                {
                    ...
                    if (GetForegroundWindow() == hAcadWnd)
                    {
                        ...
                        if (!FrameworkImeAutoSwitcher::isCommandRunning())
                        {
