...
namespace UtilBalloonNumber
{
    BalloonNumberJig::BalloonNumberJig(unsigned int num) : mNum(num)
    {
        ...
        UtilAnnotative::applyCurrentAnnotativeScale(this->mpBlockReference); // 应用当前注释性比例
        UtilBalloonNumber::syncAttributesFromDefinition(this->mpBlockReference, num); // 同步属性值
...
namespace UtilBalloonNumber
{
    ...
    void insertBalloonNumber(AcGePoint3d insPt, unsigned int num)
    {
        ...
        UtilAnnotative::applyCurrentAnnotativeScale(pBlkRef); // 应用当前注释性比例
        UtilBalloonNumber::syncAttributesFromDefinition(pBlkRef, num);
    ...
    void insertBalloonNumberBlockWithStartNumber(int num)
    {
        ...
        while (true)
        {
            UtilBalloonNumber::BalloonNumberJig jig(static_cast<unsigned int>(num));
            asPrompt.format(_(L"\n指定序号 %d 的插入点[退出(Esc)]：\n"), num);
            jig.setDispPrompt(asPrompt);

            if (jig.drag() == AcEdJig::kNormal)
            {
                UtilBalloonNumber::insertBalloonNumber(jig.getPoint(), static_cast<unsigned int>(num));
                ++num;
            }
            else
            {
    ...
    {
        AcString currVal;
        if (!UtilBalloonNumber::getBalloonAttributeValue(id, currVal))
        {
            return;
        }
        ...
        try
        {
            ...
            acutPrintf(_(L"\n偏置后气泡号编号：%d"), newVal);
            UtilBalloonNumber::updateBalloonNumberBlock(id, newVal);
        }
        catch (...)
        {
            acutPrintf(_(L"\n偏置气泡号失败，气泡号值可能不是数字"));
    ...
    bool meetCriteria(const AcString& attrValue, const AcString& criteria)
    {
        ...
        // 等于和不等于判断允许非数字
        if (strOp == UtilBalloonNumber::OperatorType::equal)
        {
        ...
        if (strOp == UtilBalloonNumber::OperatorType::notEqual1 || strOp == UtilBalloonNumber::OperatorType::notEqual2)
        {
            if (attrValue != strTarget)
            {
                return true;
        ...
        try
        {
            ...
            if (strOp == UtilBalloonNumber::OperatorType::less1 || strOp == UtilBalloonNumber::OperatorType::less2)
            {
                return iAttrValue < iTarget;
            }
            if (strOp == UtilBalloonNumber::OperatorType::greater1 || strOp == UtilBalloonNumber::OperatorType::greater2)
            {
                return iAttrValue > iTarget;
            }
            if (strOp == UtilBalloonNumber::OperatorType::lessEqual1 || strOp == UtilBalloonNumber::OperatorType::lessEqual2)
            {
                return iAttrValue <= iTarget;
            }
            if (strOp == UtilBalloonNumber::OperatorType::greaterEqual1 || strOp == UtilBalloonNumber::OperatorType::greaterEqual2)
            {
                return iAttrValue >= iTarget;
            }
        }
