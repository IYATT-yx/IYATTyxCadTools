 ...
 * @file      UtilCommon.cpp
 * @brief     通用工具模块的实现文件。
 * @details   本文件实现了 UtilCommon 模块中声明的各类辅助工具函数。
 * @author    IYATT-yx
...
namespace UtilCommon
{
	...
	{
		acdbRToS(doubleValue, AcStringValue, UtilCommon::UnitMode::Decimal, precision);

		if (forcePlusSign && doubleValue > UtilCommon::Epsilon)
		{
			AcStringValue = L"+" + AcStringValue;
		}
		else if (addSpaceIfZero && std::abs(doubleValue) <= UtilCommon::Epsilon)
		{
			AcStringValue = L" " + AcStringValue;
		}
	}
	...
	bool setCharMapFontToGDT()
	{
		...
		if (status == ERROR_SUCCESS)
		{
			DWORD valueSize = static_cast<DWORD>((wcslen(UtilCommon::CharMap::font) + 1) * sizeof(WCHAR));
			status = RegSetValueExW(
				key,
				UtilCommon::CharMap::key,
				0,
				REG_SZ,
				reinterpret_cast<const BYTE*>(UtilCommon::CharMap::font),
				valueSize
			);
			RegCloseKey(key);
		}
	...
	void startCharMapWithGDT()
	{
		...
		if (UtilCommon::setCharMapFontToGDT())
		{
			HINSTANCE inst = ShellExecuteW(
				nullptr,
				L"open",
				UtilCommon::CharMap::programName,
				nullptr,
				nullptr,
				SW_SHOWNORMAL
			);
	...
	{
		CAcModuleResourceOverride resOverride;
		AcDbObject* pObj = UtilCommon::getObject<AcDbObject>(objId, AcDb::kForRead);
		if (pObj == nullptr)
		{
			return;
		}
	...
	std::optional<std::filesystem::path> getAppSubFolder()
	{
		...
		try
		{
			...
			folderPath /= UtilCommon::loadString(IDS_PROJNAME).GetString();
	...
	Adesk::UInt16 getEntityActualColorIndex(const AcDbEntity* pEntity)
	{
		...
		{
			AcDbLayerTableRecord* pLayer = UtilCommon::getObject<AcDbLayerTableRecord>(pEntity->layerId(), AcDb::kForRead);
			if (pLayer != nullptr)
			{
